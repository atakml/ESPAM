...
class Edgeshaper():
    ...
    def __init__(self, model, x, edge_index, edge_attr, edge_weight = None, device = "cpu"):
        """ Args:
    ...
    def explain(self, M = 100, target_class = 0, P = None, deviation = None, log_odds = False, seed = None):
        ...
        for j in tqdm(range(num_edges)):
            ...
            for i in range(M):
                ...
                E_j_attr_plus = torch.index_select(self.edge_attr, dim = 0, index = retained_indices_plus)
                ...
                out = self.model(self.x, E_j_plus, E_j_attr_plus, batch=batch, edge_weights=edge_weight_j_plus)
                out_prob = None
                V_j_plus = None
                ...
                E_j_attr_minus = torch.index_select(self.edge_attr, dim = 0, index = retained_indices_minus)
                ...
                batch = torch.zeros(self.x.shape[0], dtype=int, device=self.x.device)
                out = self.model(self.x, E_j_minus,E_j_attr_minus, batch=batch, edge_weights=edge_weight_j_minus)
    ...
    def explain_with_deviation(self, M = 100, target_class = 0, P = None, deviation = None, log_odds = False, seed = None, device = "cpu"):
        ...
        batch = torch.zeros(self.x.shape[0], dtype=int, device=self.x.device)
        E = self.edge_index
        out = self.model(self.x, E,self.edge_attr, batch=batch, edge_weights=self.edge_weight)
        out_prob_real = out[0][target_class].item() if target_class is not None else out[0][0].item()
    ...
    def compute_original_predicted_probability(self):
        batch = torch.zeros(self.x.shape[0], dtype=int, device=self.x.device)
        out_log_odds = self.model(self.x, self.edge_index, self.edge_attr, batch=batch, edge_weights=self.edge_weight)
        out_prob = out_log_odds
    ...
    def compute_pertinent_positive_set(self, verbose = False):
        ...
        for i in range(1, important_edges_ranking.shape[0]+1):
            ...
            reduced_edge_attr = torch.index_select(self.edge_attr, dim = 0, index = torch.LongTensor(important_edges_ranking[0:i]).to(self.device))
            batch = torch.zeros(self.x.shape[0], dtype=int, device=self.x.device)
            out = self.model(self.x, reduced_edge_index, reduced_edge_attr, batch=batch)
            out_prob = out
            # print(out_prob)
    ...
    def compute_minimal_top_k_set(self, verbose = False):
        ...
        for i in range(important_edges_ranking.shape[0]):
            ...
            reduced_edge_attr = torch.index_select(self.edge_attr, dim = 0, index = torch.LongTensor(important_edges_ranking[i:]).to(self.device))
