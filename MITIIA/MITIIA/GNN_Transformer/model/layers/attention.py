...
if __name__ == '__main__':
    model = MultiHeadDotProductAttention_with_AttnWeights(num_heads = 2)

    key = jax.random.PRNGKey(0)
    key1, key2, key3 = jax.random.split(key, num = 3)
    
    K = jax.random.normal(key = key1, shape = (3, 10, 6))
    Q = jax.random.normal(key = key2, shape = (3, 20, 6))

    params = model.init(key3, inputs_q = Q, inputs_kv = K)
    res, state = model.apply(params, inputs_q = Q, inputs_kv = K, mutable=['intermediates'])

    print(state['intermediates']['attention_weights'][0])

    # ---------------------------------

    model = MultiHeadDotProductAttention_with_AttnWeights_and_QKV(num_heads = 2)
    ...
    params = model.init(key3, inputs_q = Q, inputs_k = K, inputs_v = K)
    _res, state = model.apply(params, inputs_q = Q, inputs_k = K, inputs_v = K, mutable=['intermediates'])

    print(state['intermediates']['attention_weights'][0])

    print('---------------')
    print(jax.tree_multimap(lambda x,y: jnp.abs(x - y), res, _res))