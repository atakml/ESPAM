...
from GNN_Transformer.make_predict import make_predict_epoch
...
def main_predict(hparams):
    ...
    logdir = os.path.join(hparams['LOGGING_DIR'], _datetime)

    data_csv = hparams['PREDICT_CSV']
    ...
    if hparams['N_PARTITIONS'] > 0:
        raise NotImplementedError('pmap is not implemented at the moment.')
        # state = flax.jax_utils.replicate(state)
        # predict_epoch = make_predict_epoch_pmap(logger = logger, loader_output_type = hparams['LOADER_OUTPUT_TYPE'])
    else:
        predict_epoch = make_predict_epoch(logger = logger, loader_output_type = hparams['LOADER_OUTPUT_TYPE'], return_intermediates = hparams['RETURN_INTERMEDIATES'])
    ...
    if hparams['RETURN_INTERMEDIATES']:
        predictions_np, intermediates = predictions_np
        with open("intermediates", "wb") as pkl:
            pickle.dump(intermediates, pkl)
    ...
    # Save predictions:
    print('Predictions are going to be saved to: {}'.format(os.path.join(logdir, 'predict.csv')))
