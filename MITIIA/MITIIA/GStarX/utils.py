...
def graph_build_remove(X, edge_index, edge_attr, node_mask: torch.Tensor):
    """subgraph building through removing the unselected nodes from the original graph"""
    ret_X = X[node_mask == 1]
    ret_edge_index, ret_edge_attr = subgraph(node_mask.bool(), edge_index, edge_attr, relabel_nodes=True)
...
def sample_subgraph(
    ...
    sampled_edge_index, sampled_edge_attr = subgraph(
        sampled_nodes, data.edge_index, data.edge_attr, relabel_nodes=True, num_nodes=data.num_nodes
    )
    sampled_data = Data(x=sampled_x, edge_index=sampled_edge_index, edge_attr=sampled_edge_attr)
...
class MaskedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        masked_x, masked_edge_index, masked_edge_attr = self.subgraph_building_func(
            self.x, self.edge_index, self.edge_attr, self.mask[idx]
        )
        masked_data = Data(x=masked_x, edge_index=masked_edge_index, edge_attr=masked_edge_attr)
        return masked_data
    def len(self):
        return self.mask.shape[0]
