...
from GraphSVX.src.models import LinearRegressionModel
from GraphSVX.src.plots import (denoise_graph, k_hop_subgraph, log_graph,
...
class GraphSVX():
    ...
    def explain_graphs(self,
        ...
        for graph_index in graph_indices:
            ...
            if self.gpu:
                ...
                self.model = self.model.to(device)
                with torch.no_grad():
                    A = dense_to_sparse(self.data.edge_index)[0]
                    true_pred = softmax(self.model(self.data.x[0].cuda(),
                        A))[0].max(dim=0)[1]
            else:
                with torch.no_grad():
                    A = dense_to_sparse(self.data.edge_index)[0]
                    true_pred = softmax(self.model(self.data.x[0],
                        A))[0].max(dim=0)[1]
            # Remove node v index from neighbours and store their number in D
    ...
    def graph_classification(self, graph_index, num_samples, D, z_, args_K, args_feat, true_pred):
        ...
        for (key, ex_nei) in tqdm(excluded_nei.items()):
            ...
            if self.gpu:
                with torch.no_grad():
                    proba = softmax(self.model(X.cuda(), edge_index.cuda()))
            else:
                with torch.no_grad():
                    proba = softmax(self.model(X, edge_index))
    ...
    def WLS(self, z_, weights, fz, multiclass, info):
        ...
        if info:
            ...
            print('weighted r2: ', r2_score(fz.numpy(), y_pred, sample_weight=weights.numpy()))
