...
def get_associated_game_matrix_P(coalitions, n, adj):
    ...
    for i, s in enumerate(coalitions):
        ...
        num_cc, labels = cc(adj[idx_s, :][:, idx_s].cpu())
        cc_s = []
...
def sample_subgraph(
    ...
    if sample_method == "khop":
        ...
        sampled_nodes = torch.tensor(list(sampled_nodes)).to(data.x.device)

    elif sample_method == "random":  # randomly pick #max_sample_size nodes
        sampled_nodes = torch.randperm(data.num_nodes).to(data.x.device)[:max_sample_size]
    else:
