...
class GStarX(object):
    ...
    def explain(
        ...
        target_class = self.model(data.x, data.edge_index).argmax(-1).item()
        char_func = get_char_func(
            self.model, target_class, self.payoff_type, self.payoff_avg
        )
        if data.num_nodes < self.max_sample_size:
            scores = self.compute_scores(data, adj, char_func, superadditive_ext).to(self.device)
        else:
            scores = torch.zeros(data.num_nodes).to(self.device)
            counts = torch.zeros(data.num_nodes).to(self.device)
            ...
            while not counts.all() or i < num_samples:
                ...
                scores[sampled_nodes] += sampled_scores.to(self.device)
