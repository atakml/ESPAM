...
class Bridge_model(nn.Module):
    ...
    def forward(self, x, adj):
        data= Data(x=x, edge_index=adj,batch=torch.zeros(len(x), dtype=int))
        output = self.model.forward(data)
        return output, None


def gnn_explainer_bridge(model, **kwargs):
    pass
    """
    separate the dataset to 1 mol and write the adj, label... 
    """
    dset = [el for el in Dl(kwargs["trainset"].dataset, batch_size=1)]
    arg= args.arg_parse()
    arg.hidden_dim=kwargs["hidden_dims"][0]
    arg.output_dim=kwargs["output_dim"]
    arg.num_classes=kwargs["output_dim"]
    arg.num_gc_layers=len(kwargs["hidden_dims"])

    model = Bridge_model(model)
    #for el in dset:
    #    xx= model(el)
    for el in dset:
        pred=model(el.x, el.edge_index)[0].detach()
        xpl= Explainer(Bridge_model(model), el.edge_index, el.x, el.y, [[pred]], torch.zeros(el.x.shape), arg, graph_mode=True)
        xpl.explain_graphs([0])
...
def neighborhoodsfn(adj, n_hops, use_cuda):
...
class Explainer:
    def __init__(
        ...
        self.neighborhoods = None if self.graph_mode else neighborhoodsfn(adj=self.adj, n_hops=self.n_hops, use_cuda=use_cuda)
        self.args = args
        self.writer = writer
