...
from ExplanationEvaluation.models.LinearRegressionModel import LinearRegressionModel
from ExplanationEvaluation.plots.plots_SVX import (denoise_graph, k_hop_subgraph, log_graph,
...
class GraphSVX(BaseExplainer):
    ...
    def graph_classification(self, graph_index, num_samples, D, z_, args_K, args_feat, true_pred):
        ...
        if args_feat == 'Null':
            ...
            av_feat_values = self.features.mean(dim=0).mean(dim=0)
        ...
        for (key, ex_nei) in (excluded_nei.items()):
            ...
            if self.gpu:
                with torch.no_grad():
                    proba = self.model_to_explain(X.cuda(), A.cuda()).softmax(axis=1)
            else:
                with torch.no_grad():
                    proba = self.model_to_explain(X, A).softmax(axis=1)
