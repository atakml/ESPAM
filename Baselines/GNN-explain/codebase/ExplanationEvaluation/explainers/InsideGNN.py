...
class InsideGNN(BaseExplainer):
    ...
    def prepare(self, indices):
        ...
        self.rules = labeled_rules(self.config.dataset,self.motif,self.negation)
    ...
    def _call_subgroup(self):
        file_name = self.config.dataset+"_"+self.config.model+"_"+self.config.explainer+"_activation"
        cp_file_name = self.config.dataset+"_activation"
        code_dir = "ExplanationEvaluation/PatternMining/"
        cp_cmd= "cp results/trace/"+ file_name+".csv " +code_dir+cp_file_name+".csv"
        os.system(cp_cmd)

        for layer in range(4):
            command1 = "(cd "+code_dir+"; python  "+"pretraitement.py -i " + cp_file_name+ ".csv -l "+ str(layer)+")"
            os.system(command1)
        for layer in range(4):
            command2 =  "(cd "+code_dir+ ";python  "+"si_activation_pattern.py -i "+ cp_file_name+"_pretraite_"+str(layer)+".csv -l "+str(layer)+ " -k 1 -s 1 -m 0"+")"
            command3 =  "(cd "+code_dir+";python "+"si_activation_pattern.py -i "+ cp_file_name+"_pretraite_"+str(layer)+".csv -l "+str(layer)+ " -k 5 -s 1 -m 1"+")"
            os.system(command2)
            os.system(command3)
        ...
        names= {"ba2": ("ba2"),
                ...
                "Mutag": ("mutag")}
        name = names[self.config.dataset]

        file = "ExplanationEvaluation/datasets/activations/"+ name+"/"+name+self.negation+"_encode_motifs.csv"
    ...
    def _embedding_to_df(self, labels =True):
        ...
        inputs = pd.DataFrame(atoms.numpy(), columns=["l_3c_"+str(i) for i in range(len(atoms[0]))])
