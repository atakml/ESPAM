...
class OtLoader:
    def __init__(self, model, graphs, dataset_name, target_rule, method):
        self.model = model
        self.graphs = graphs
        self.dataset_name = dataset
        self.target_rule = target_rule  # for the moment the target_rule should be in the form (layer, target, no)
        self.method = method
        ...
        self.atoms_mutag = {0: "C", 1: "O", 2: "Cl", 3: "H", 4: "N", 5: "F", 6: "Br", 7: "S", 8: "P", 9: "I", 10: "Na",
                            11: "K",
                            12: "Li", 13: "Ca"}
        self.BBBPs = ["C", "N", "O", "S", "P", "BR", "B", "F", "CL", "I", "H", "NA", "CA"]
        self.features_dict = self.atoms_aids
        self.G = self.load_median_graph_from_file()
        self.rule_evaluator = RuleEvaluator(self.model, self.dataset_name, self.graphs, self.target_rule[0])


    @staticmethod
    def __add_edges(graph, edge_index):
        """
    ...
    def __add_features_from_matrix(self, graph, features_matrix, dict_features):
        """
        ...
        for i in range(features_matrix.shape[0]):  # for each node
            for j in range(features_matrix.shape[1]):  # for each feature
                if features_matrix[i][j] == 1:
                    try:
                        graph.nodes[i]['label'] = self.BBBPs[j]
                    except (KeyError):
                        print(i, " ")

    def load_median_graph_from_file(self):
        with open(f"median_{self.dataset_name}_{self.method}_" + "_".join(map(str, self.target_rule)) + ".json") as f:
            data = json.load(f)
        ...
        G = nx.Graph()
        G.add_node(0)
        self.__add_edges(G, edge_index)
        self.__add_features_from_matrix(G, features_matrix)
        return G


def run_masking_ot(dataset, method=None, small=False):
    ...
    rules = range(Number_of_rules[dataset])  # to be changed
    rules = [(1,1,0), (1,0,1)]
    scores = list()
    nsteps = 5000
    nxp = 6
    ...
    gdset = graph_dset([graphs, features], atoms)  # to be checked
    ...
    for rule in tqdm(rules):
        # pay attention rule is only the index of the rule
        for x in [0]:  # (range(nxp)):
            explainer = OtLoader(model, (graphs, features, labels), dataset, rule, method)  # loading masks
            best_ego = explainer.G  # mask of the rule
            masks[rule] = explainer.model(best_ego).detach().softmax(dim=1)[0, 1]

            best_ego.nodes[0]["label"] = "center" + str(best_ego.nodes[0]["label"])

            egos[rule] = best_ego

    for i, (adj, f, (emb, pred), g) in tqdm(
            list(enumerate(zip(graphs, features, fid_precompute[rule[0]], gdset)))):
        # if i not in [134]:
...
def run_masking(dataset, method=None, small=False):
    ...
    for rule in tqdm(rules):
        ...
        for x in [0]:  # (range(nxp)):
            ...
            if small:
                ...
                file = "results/" + dir_name + "/dataset_" + dataset + "_rule_" + str(
                    rule) + "metric_" + metric + "xp_" + str(x) + "steps_" + str(nsteps) + "nxp_" + str(
                    nxp) + "ratio_" + str(0.5) + ".pkl"
    ...
    for i, (adj, f, (emb, pred), g) in tqdm(
            list(enumerate(zip(graphs, features, fid_precompute[explainer.rule_evaluator.get_layer()], gdset)))):
        # if i not in [134]:
        df = get_fidelity(explainer.rule_evaluator, metric, dataset, i, emb, pred, adj, f, g, egos, masks)
...
def graph_dset(dset, atoms):
    ...
    for graph, features in zip(*dset):
        try:
            ...
            dense_adj =np.zeros((features.shape[0], features.shape[0]))
...
def get_fidelity(evaluator, metric, dataset, id, embs, pred, adj, feat, graph, ego, masks, mode=None):
    ...
    for rulenumber, egg in ego.items():
        ...
        acts = [RuleEvaluator.activate_static(target_rule, emb) for emb in embs[target_rule[0]]]
        ...
        for egid, eg in enumerate([egg]):
            ...
            if len(g2):
                ...
                try:
                    pred3 = evaluator.get_output(graph.subgraph(remove_nodes)).softmax(dim=1)[0, 1]
                except:
                    pred3 = torch.tensor(0)
